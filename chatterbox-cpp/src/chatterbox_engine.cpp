...
struct Engine::Impl {
    ...
    explicit Impl(const EngineOptions & o)
        ...
        // MTL variant requires a non-empty language code.
        if (model.hparams.variant == CHBX_VARIANT_MTL && opts.language.empty()) {
            free_model();
            throw std::runtime_error(
                "Engine: t3_mtl GGUF requires a non-empty language code in "
                "EngineOptions::language (tier-1: en, es, fr, de, it, pt, "
                "nl, pl, tr, sv, da, fi, no, el, ms, sw, ar, ko).  "
    ...
    std::vector<int32_t> run_t3_mtl(const std::string & text) {
        if (model.mtl_tokenizer_json.empty()) {
            throw std::runtime_error(
                "Engine: this t3_mtl GGUF has no embedded MTL tokenizer");
        }

        mtl_tokenizer mtl_tok;
        if (!mtl_tok.load_from_json(model.mtl_tokenizer_json)) {
            throw std::runtime_error("Engine: failed to load MTL tokenizer");
        }
        if (!mtl_tok.is_language_supported(opts.language)) {
            throw std::runtime_error(
                "Engine: unsupported MTL language: " + opts.language);
        }

        // Tokenize with language prefix + start/stop text tokens.
        ...
        if (!eval_prompt_mtl(model, allocr, n_threads, text_tokens,
            ...
            throw std::runtime_error("Engine: MTL prompt eval failed");
        }
        ...
        for (int i = 0; i < opts.n_predict; ++i) {
            if (cancel_flag.load(std::memory_order_relaxed)) {
                throw std::runtime_error("Engine: synthesis cancelled during MTL T3 decode");
            }
            if (current == model.hparams.stop_speech_token) break;
            if (n_past + 1 > model.hparams.n_ctx) break;
            if (!eval_step_mtl(model, allocr, n_threads, n_past, current,
                               logits_c, logits_u)) {
                throw std::runtime_error("Engine: MTL step eval failed");
            }
            ++n_past;
    ...
    SynthesisResult synthesize(const std::string & text,
        ...
        std::vector<int32_t> speech_tokens = (model.hparams.variant == CHBX_VARIANT_MTL)
            ? run_t3_mtl(text)
            : run_t3(text);
