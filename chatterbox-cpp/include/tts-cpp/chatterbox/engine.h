...
struct EngineOptions {
    ...
    // MTL-specific (t3_mtl variant).  Ignored when the loaded GGUF is Turbo.
    std::string language;              // Language code (required for MTL: en, es, fr, de, it, ...)
    float       cfg_weight   = 0.5f;   // Classifier-free guidance strength
    float       min_p        = 0.05f;  // Min-probability warp (0 = off)
    float       exaggeration = 0.5f;   // Emotion-adv scalar [0, 1]
